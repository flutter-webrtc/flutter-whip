...
class LocalStream {
  ...
  void _publishTrack({required MediaStreamTrack track}) async {
    if (_pc != null) {
  ...
  Future<void> publish(RTCPeerConnection pc) async {
    _pc = pc;
    _stream.getTracks().forEach((track) async => _publishTrack(track: track));
  }
  ...
  Future<void> switchDevice(String kind, {required String deviceId}) async {
    ...
    await _updateTrack(next: next, prev: prev);
  }
  ...
  Future<void> unmute(String kind) async {
    ...
    await _updateTrack(next: track, prev: prev);
  }
}
...
RemoteStream makeRemote(MediaStream stream, WHIP whip) {
  ...
  var select = () {
    ...
    if (whip.api == null ||
        (whip.api != null &&
            whip.api?.state != RTCDataChannelState.RTCDataChannelOpen)) {
      /// queue call if we aren't open yet
      whip.onAPIOpen = () {
        whip.api?.send(RTCDataChannelMessage(jsonEncoder.convert(call)));
      };
    }
